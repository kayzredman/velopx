    ...
    <div className="min-h-screen bg-[#060F1E] flex">
      <aside className="fixed top-0 left-0 h-screen w-[264px] bg-[#0A1628] border-r border-[#1E2E48] flex flex-col z-40">
        <div className="px-5 py-5 border-b border-[#1E2E48]">
          <Link href="/insurer" className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <span className="block text-rose-400 text-[10px] uppercase tracking-widest leading-none mt-0.5">Insurer</span>
            </div>
          </Link>
        </div>
        ...
        <nav className="flex-1 overflow-y-auto px-3 py-4 flex flex-col gap-1">
          ...
          <p className="text-[#3D5068] text-[9px] font-bold uppercase tracking-[0.15em] px-3 mt-5 mb-1">Admin</p>
          {ADMIN_NAV.map((item) => (
            <NavItem key={item.href} item={item} />
          ))}
        </nav>
        ...
        <div className="px-5 py-4 border-t border-[#1E2E48] flex items-center gap-3">
          ...
          <div className="min-w-0">
            <p className="text-white text-xs font-medium truncate">Insurance Portal</p>
            <p className="text-[#8A97AA] text-[10px] truncate">VelopX Insurer</p>
          </div>
        </div>
      </aside>
